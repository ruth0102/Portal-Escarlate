...
import { sendPasswordRecoveryEmail } from './lib/send-password-recovery-email.js'
...
function validatePasswordRecoveryPayload(payload) {
  ...
  if (typeof payload.recoveryUrl !== 'string' || !payload.recoveryUrl.trim()) {
    return 'URL de recuperacao ausente.'
  }
...
function validateEventPayload(payload) {
  ...
  if (payload.type === 'email.password_recovery_requested') {
    return validatePasswordRecoveryPayload(payload.payload)
  }
  ...
  try {
    if (payload.type === 'email.password_recovery_requested') {
      await sendPasswordRecoveryEmail({
        ...
        recoveryUrl: payload.payload.recoveryUrl.trim(),
      })
      void publishEventSafely({
        type: 'email.password_recovery_sent',
        source: 'email-service',
        payload: {
          to: payload.payload.to.trim(),
    ...
    json(response, 502, {
      message: `Nao foi possivel processar o evento de e-mail: ${detail}`,
    })
  }
}
