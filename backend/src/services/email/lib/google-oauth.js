...
export async function testGoogleRefreshToken(refreshToken) {
  ...
  if (!accessToken) {
    throw new Error('Google nao retornou access token para esta conexao.')
  }
  ...
  if (!scopes.includes('https://www.googleapis.com/auth/gmail.send')) {
    throw new Error('Conexao Gmail sem permissao de envio. Reconecte o Google para autorizar gmail.send.')
  }
}
