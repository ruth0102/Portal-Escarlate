...
import { flattenFieldErrors, loginSchema, passwordSchema } from '../../lib/auth/validation.js'
import {
...
function validateInternalPasswordUpdatePayload(payload) {
  if (!payload || typeof payload !== 'object') {
    return 'Dados de atualizacao ausentes.'
  }
  ...
  const parsedPassword = passwordSchema.safeParse(payload.password)

  if (!parsedPassword.success) {
    return parsedPassword.error.issues[0]?.message ?? 'Senha invalida.'
  }
...
async function handleInternalUpdatePassword(request, response) {
  let payload
  ...
  try {
    ...
    json(response, error?.status === 413 ? 413 : 400, {
      message:
        error?.status === 413
          ? 'Dados de atualizacao muito grandes.'
          : 'Nao foi possivel ler os dados de atualizacao.',
    })
    return
  }

  const errorMessage = validateInternalPasswordUpdatePayload(payload)
  ...
  try {
    ...
    const updatedUser = await updateUserPasswordByEmail(payload.email, passwordHash)

    if (!updatedUser) {
      json(response, 404, { message: 'Usuario nao encontrado.' })
      return
    }
    ...
    json(response, 200, { message: 'Senha atualizada com sucesso.' })
  } catch (error) {
    console.error('[auth-service] Failed to update user password', error)
    json(response, 500, { message: 'Nao foi possivel atualizar a senha.' })
  }
}
...
async function route(request, response) {
  ...
  if (request.method === 'POST' && url.pathname === '/internal/auth/users/password') {
    if (!validateInternalRequest(request, response, json)) {
      return
    }

    await handleInternalUpdatePassword(request, response)
    return
  }
